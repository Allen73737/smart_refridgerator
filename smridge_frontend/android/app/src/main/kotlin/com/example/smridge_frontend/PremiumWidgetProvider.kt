...
class PremiumWidgetProvider : HomeWidgetProvider() {
    companion object {
        private const val ACTION_BLINK = "com.example.smridge_frontend.ACTION_BLINK_PREM"
    }
        ...
        for (appWidgetId in appWidgetIds) {
            try {
                val views = RemoteViews(context.packageName, R.layout.widget_premium).apply {
                    ...
                    val status = widgetData.getString("status", "ACTIVE") ?: "ACTIVE"
                    ...
                    if (shouldBlink) {
                        ...
                        Handler(Looper.getMainLooper()).postDelayed({
                            try {
                                val stopViews = RemoteViews(context.packageName, R.layout.widget_premium)
    ...
    private fun scheduleBlinkAlarm(context: Context, targetTimestamp: Long) {
        val intent = Intent(context, PremiumWidgetProvider::class.java).apply {
            action = ACTION_BLINK
        }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == ACTION_BLINK) {
            ...
            val componentName = android.content.ComponentName(context, PremiumWidgetProvider::class.java)
